...
void PTUpdateEntry( int arr[MAXDIM][MAXDIM+1], int i, int j, int
value )
{
...
twork[tnum]++ ;
arr[i][j] = value ;
if ( ( (tnum == SLOW_THREAD) && (rand()%1 == 0) ) || ( (tnum !=
SLOW_THREAD) && (rand()%100 == 0) ) )
{
...
printf( "numt=%d, status=%s, workload=[%d", numt, status,
twork[0] ) ;
for( i = 1 ; i < numt ; i++ )
printf( ",%d", twork[i] ) ;
printf( "], time=%g\n", time2-time1 ) ;
